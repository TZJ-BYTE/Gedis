...
func (v *Version) clone() *Version {
	...
	nv := &Version{Files: make([][]*FileMetadata, len(v.Files))}
	for i := range v.Files {
		if len(v.Files[i]) == 0 {
			continue
		}
...
func OpenVersionSet(dbDir string, maxLevels int) (*VersionSet, error) {
	...
	err := os.MkdirAll(versionDir, 0750)
	if err != nil {
		return nil, err
	}
...
func (vs *VersionSet) createManifest() error {
	...
	var err error
	vs.manifestFile, err = os.OpenFile(manifestPath, os.O_CREATE|os.O_RDWR|os.O_APPEND, 0600)
	if err != nil {
		return err
	}
	...
	// 写入 CURRENT 文件
	err = os.WriteFile(currentPath, []byte(ManifestFileName), 0600)
	if err != nil {
		return err
	}
...
func (vs *VersionSet) recoverFromManifest(manifestPath string) error {
	...
	for {
		...
		case 1: // 新增文件
			...
			if len(fm.SmallestKey) == 0 && len(fm.LargestKey) == 0 && fm.Size > 0 {
				sstablePath := filepath.Join(vs.dbDir, "sstable", fmt.Sprintf("%06d.sstable", fm.FileNum))
				reader, err := OpenSSTableForReadWithCache(fm.FileNum, sstablePath, nil, nil)
				if err == nil {
					...
					reader.Close()
					if smallest != nil {
						fm.SmallestKey = smallest
						fm.LargestKey = append([]byte(nil), largest...)
					}
				}
			}
...
func (vs *VersionSet) LogAddFile(fm *FileMetadata) error {
	...
	// 先更新内存中的版本
	vs.currentVersion = vs.currentVersion.clone()
	vs.currentVersion.AddFile(fm.Level, fm)
